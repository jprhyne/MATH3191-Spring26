...
\begin{document}
    ...
    \begin{frame}{Dot Product}
        ...
        \end{defn}\pause
        \begin{theorem}
            The \bText{dot product} is an inner product for $\R^n$. (Not $\C^n$. Think about why this might be!)
        \end{theorem}\pause
        \begin{proof}
    ...
    \begin{frame}{Example of a Norm}
        If $V=\C^n$ and $\F=\C$, then the induced norm is often called the ``Euclidean Norm'' and
        denoted as follows\pause
        \[
            \norm{\vec{x}}_2 = \sqrt{\vec{x}^*\vec{x}}\pause = \sqrt{\sum_{k=1}^n|x_k|^2}
        \]\pause
        For this course, we will consider only this norm unless stated otherwise. (If $V=\R^n$, then
        this is the same as omitting the absolute value!)
