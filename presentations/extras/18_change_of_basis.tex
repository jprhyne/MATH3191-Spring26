...
\begin{document}
    \begin{frame}{Change of Coordinate Matrix in $\R^n$}
        ...
        the matrix
        \[
            P_\B = \bMat{\bCoord{\vec{b}_1}{\C} & \dots & \bCoord{\vec{b}_n}{\C}}
        \]
        Represents our basis vectors as an array of real numbers. Where $\C$ is the ``standard''
        basis for our vector space. (In $\R^n$, these are the columns of the identity matrix)\pause\

        What does this mean for $V=\R^n$?\pause\ Let's compute $P_\C\vec{x}$!\pause\
        \[
            P_\B\vec{x} = x_1\bCoord{\vec{b}_1}{\C} + \dots + x_n\bCoord{\vec{b}_n}{\C}
        \]\pause
