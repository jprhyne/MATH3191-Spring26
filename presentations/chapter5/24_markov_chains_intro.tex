...
\begin{document}
    ...
    \begin{frame}{Eigenvalues of Stochastic Matrices Part 1}
        ...
        \begin{proof}
            ...
            \]
            So, $\lambda=1$ is an eigenvalue of $A$.
        \end{proof}
    \end{frame}
    \begin{frame}{Eigenvalues of Stochastic Matrices Part 2}
        ...
        \begin{proof}
            ...
            \]\pause\
            If we pick the index $j$ such that $\abs{x_i}\leq\abs{x_j}$ for all $i$,
        ...
        \begin{columns}
            ...
            \begin{enumerate}
                \pause\item If we assign each node a number from $1$ to $n$ where $n$ is
                    ...
                    \]
                    \vspace{-12pt}
                \pause\item $P_{ji}$ is the probability of going from state $i$ to state $j$.
                \pause\item $P$ is also a stochastic matrix!
            \end{enumerate}
        \end{columns}
    \end{frame}
    \begin{frame}{Markov Chains in a Nutshell}
        \instring{handout}{\jobname}{
            \begin{tcolorbox}
                ...
                a large number of times under the same condition
                    \begin{itemize}
                        \item Each experiment has the same $n$ possible outcomes
                        \item There is some set of probabilistic rules that determine how the system moves
                            from one state to another
                        \item These probabilities only depend on the current state
                    \end{itemize}
            \end{tcolorbox}
            This can be applied to many fields including: Epidemiology, Finance, Statistics, etc.
        }{
            \instring{beforeClass}{\jobname}{
                \begin{tcolorbox}
                    An intuitive view of a Markov Chain is a math model describing some kind of ``experiment''
                    a large number of times under the same condition
                        \begin{itemize}
